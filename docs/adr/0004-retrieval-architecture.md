...
- Search-all-by-default with first-class filters: unknown filter values return empty results. The planned `kv search` CLI validates filter values against `indexed_sources` (not yet implemented as of 0.5).
